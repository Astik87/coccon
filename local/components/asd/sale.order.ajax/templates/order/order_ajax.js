	...
	BX.Sale.OrderAjaxComponent = {
		...
		showError: function (node, msg, border) {
			...
			if (errorContainer && msg.length) {
				...
				errorContainer.appendChild(BX.create('DIV', {
					html: msg
				}));
		...
		showBlockWarning: function (node, warnings, hide) {
			...
			if (errorNode) {
				...
				warningNode = BX.create('DIV', {
					props: {
						className: 'alert alert-warning' + (!!hide ? ' alert-hide' : ' alert-show')
					},
					html: warnStr
				});
			...
			if (pagination.pages.length > 1) {
				navigation.push(
					BX.create('LI', {
						...
						props: {
							className: 'bx-pag-prev'
						},
						html: pagination.pageNumber == 1 ?
							'<span>' + this.params.MESS_NAV_BACK + '</span>' : '<a href=""><span>' + this.params.MESS_NAV_BACK + '</span></a>',
						events: {
				...
				navigation.push(
					BX.create('LI', {
						...
						props: {
							className: 'bx-pag-next'
						},
						html: pagination.pageNumber == pagination.pages.length ?
							'<span>' + this.params.MESS_NAV_FORWARD + '</span>' : '<a href=""><span>' + this.params.MESS_NAV_FORWARD + '</span></a>',
						events: {
							click: BX.proxy(this.doPagination, this)
						}
					})
				...
				paginationNode = BX.create('DIV', {
					props: {
						className: 'bx-pagination' + (colorTheme ? ' bx-' + colorTheme : '')
					},
					children: [
						BX.create('DIV', {
							props: {
								className: 'bx-pagination-container'
							},
		...
		doPagination: function (e) {
			var target = e.target || e.srcElement,
				node = target.tagName == 'LI' ? target : BX.findParent(target, {
					tagName: 'LI'
				}),
		...
		showDeliveryItemsPage: function (page) {
			...
			var deliveryItemsContainerRow = BX.create('div', {
				props: {
					className: 'row'
				}
			});
		...
		showPaySystemItemsPage: function (page) {
			...
			var paySystemItemsContainerRow = BX.create('div', {
				props: {
					className: 'row'
				}
			});
		...
		initOptions: function () {
			...
			this.propertyCollection = new BX.Sale.PropertyCollection(BX.merge({
				publicMode: true
			}, this.result.ORDER_PROP));
			this.fadedPropertyCollection = new BX.Sale.PropertyCollection(BX.merge({
				publicMode: true
			}, this.result.ORDER_PROP));
		...
		clickNextAction: function (event) {
			var target = event.target || event.srcElement,
				actionSection = BX.findParent(target, {
					className: "bx-active"
				}),
		...
		getBlockFooter: function (node) {
			...
			if (!isLastNode) {
				buttons.push(
					BX.create('button', {
						props: {
							href: 'javascript:void(0)',
							className: 'pull-right btn btn-primary pl-3 pr-3'
						},
			...
			node.appendChild(
				BX.create('DIV', {
					props: {
						className: 'row bx-soa-more'
					},
					children: [
						BX.create('DIV', {
							props: {
								className: 'bx-soa-more-btn col'
							},
							children: buttons
						})
					]
		...
		getNewContainer: function (notFluid) {
			return BX.create('DIV', {
				props: {
					className: 'bx-soa-section-content' + (!!notFluid ? '' : ' container-fluid')
				}
			});
		},
			...
			if (showSmsConfirm) {
				...
				authFormNodes.push(
					BX.create('DIV', {
						...
						style: {
							display: this.authGenerateUser ? 'none' : ''
						},
						children: [
				...
				if (this.result.AUTH.captcha_registration == 'Y') {
					authFormNodes.push(BX.create('DIV', {
						...
						children: [
							BX.create('DIV', {
								props: {
									className: 'bx-authform-label-container'
								},
								children: [
									BX.create('SPAN', {
										props: {
							...
							BX.create('DIV', {
								props: {
									className: 'bx-authform-input-container'
								},
								children: [
									BX.create('INPUT', {
										attrs: {
			...
			authContent.appendChild(
				BX.create('DIV', {
					...
					children: [
						BX.create('DIV', {
							props: {
								className: 'bx-soa-reference col'
							},
							children: [
		...
		toggleAuthForm: function (event) {
			...
			section.appendChild(
				BX.create('DIV', {
					props: {
						className: 'bx-soa-section-title-container'
					},
					children: [
						BX.create('div', {
							attrs: {
		...
		editBasketItemsHeader: function (basketItemsNode) {
			...
			var headers = [
					BX.create('DIV', {
						...
						children: [
							BX.create('DIV', {
								props: {
									className: 'bx-soa-item-td-title'
								},
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				headers.push(
					BX.create('DIV', {
						props: {
							className: 'bx-soa-item-td bx-soa-item-properties' + (toRight ? ' bx-text-right' : '')
						},
						style: {
							paddingBottom: '5px'
						},
				...
				if (basketColumnIndex == 4 && this.result.GRID.HEADERS[i + 1]) {
					headers.push(BX.create('DIV', {
						props: {
							className: 'bx-soa-item-nth-4p1'
						}
					}));
					basketColumnIndex = 0;
				}
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {
						className: 'bx-soa-item-tr d-none d-md-table-row'
					},
					children: headers
				})
			);
		...
		createBasketItem: function (basketItemsNode, item, index, active) {
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
				...
				++basketColumnIndex;
				if (basketColumnIndex == 4 && this.result.GRID.HEADERS[i + 1]) {
					otherColumns.push(BX.create('DIV', {
						props: {
							className: 'bx-soa-item-nth-4p1'
						}
					}));
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {
						className: 'bx-soa-item-tr bx-soa-basket-info' + (index == 0 ? ' bx-soa-item-tr-first' : '')
					},
					children: cols
				})
			);
						...
						children: [
							BX.create('DIV', {
								...
								children: [
									...
									BX.create('DIV', {
										props: {
											className: 'bx-soa-item-info-block'
										},
										children: [
											BX.create('TABLE', {
												props: {
													className: 'bx-soa-info-block'
												},
		...
		createBasketItemImg: function (data) {
			...
			logoNode = BX.create('DIV', {
				props: {
					className: 'bx-soa-item-imgcontainer'
				}
			});
			...
			return BX.create('DIV', {
				props: {
					className: 'bx-soa-item-img-block'
				},
				children: [logoNode]
			});
		},
		...
		createBasketItemContent: function (data) {
			...
			return BX.create('DIV', {
				...
				children: propsNodes.length ? [
					BX.create('DIV', {
						props: {
							className: 'bx-soa-item-title'
						},
						html: titleHtml
					}),
					BX.create('DIV', {
						props: {
							className: 'bx-scu-container'
						},
						children: propsNodes
					})
				] : [
		...
		createBasketItemColumn: function (column, allData, active) {
			...
			if (column.id === 'PRICE_FORMATED') {
				...
				if (this.options.showPriceNotesInBasket && active) {
					textNode.appendChild(BX.create('BR'));
					textNode.appendChild(BX.create('SMALL', {
						text: data.NOTES
					}));
				}
				...
				if (BX.type.isArray(columnData)) {
					...
					if (val.length) {
						textNode.appendChild(
							BX.create('DIV', {
								props: {
									className: 'bx-scu-list'
								},
								children: [BX.create('UL', {
									props: {
										className: 'bx-scu-itemlist'
									},
									children: val
								})]
							})
			...
			return BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {
							className: 'bx-soa-item-td-title d-none d-md-block d-lg-none'
						},
						text: column.name
					}),
					textNode
					...
					for (i in props) {
						if (props.hasOwnProperty(i)) {
							...
							propsNodes.push(
								BX.create('TR', {
									props: {
										className: 'bx-soa-info-line'
									},
									children: [
										BX.create('TD', {
											props: {
												className: 'bx-soa-info-title'
											},
											text: name + ':'
										}),
										BX.create('TD', {
		...
		getImageContainer: function (link, source) {
			return BX.create('LI', {
				props: {
					className: 'bx-img-item'
				},
				children: [
					BX.create('DIV', {
						props: {
							className: 'bx-scu-itemColorBlock'
						},
						children: [
							BX.create('DIV', {
								props: {
									className: 'bx-img-itemColor'
								},
								style: {
		...
		getCouponNode: function (coupon, active) {
			...
			return BX.create('STRONG', {
				...
				children: active ? [
					...
					BX.create('SPAN', {
						props: {
							className: 'bx-soa-coupon-remove'
						},
						events: {
							click: BX.delegate(function (e) {
								var target = e.target || e.srcElement,
									coupon = BX.findParent(target, {
										tagName: 'STRONG'
									});
		...
		getCouponsLabel: function (active) {
			return BX.create('DIV', {
				props: {
					className: 'bx-soa-coupon-label'
				},
				children: active ? [BX.create('LABEL', {
		...
		addCoupon: function (coupon) {
			...
			for (var i = 0; i < couponListNodes.length; i++) {
				...
				couponListNodes[i].appendChild(this.getCouponNode({
					text: coupon
				}, true, 'bx-soa-coupon-item-danger'));
			}
		},
		...
		getDeliveryLocationInput: function (node) {
			...
			if (location && location[0] && location[0].output) {
				...
				insertedLoc = BX.create('DIV', {
					attrs: {
						'data-property-id-row': locationId
					},
					props: {
			...
			if (altProperty) {
				altNode = BX.create('DIV', {
					attrs: {
						'data-property-id-row': altProperty.ID
					},
					props: {
						className: "form-group bx-soa-location-input-container"
					}
		...
		editPaySystemItems: function (paySystemNode) {
			...
			var paySystemItemsContainer = BX.create('DIV', {
					props: {
						className: 'order-md-1 order-2 col-md-7 bx-soa-pp-item-container'
					}
				}),
				paySystemItemsContainerRow = BX.create('DIV', {
					props: {
						className: 'row'
					}
				}),
		...
		createPaySystemItem: function (item) {
			...
			logoNode = BX.create('DIV', {
				props: {
					className: 'bx-soa-pp-company-image'
				}
			});
			...
			itemNode = BX.create('DIV', {
				props: {
					className: 'bx-soa-pp-company col-6'
				},
			...
			if (currentPaySystem) {
				...
				if (this.params.SHOW_PAY_SYSTEM_INFO_NAME == 'Y') {
					subTitle = BX.create('DIV', {
						props: {
							className: 'bx-soa-pp-company-subTitle'
						},
				...
				label = BX.create('DIV', {
					props: {
						className: 'bx-soa-pp-company-logo'
					},
					children: [
						BX.create('DIV', {
							props: {
				...
				title = BX.create('DIV', {
					props: {
						className: 'bx-soa-pp-company-block'
					},
					children: [BX.create('DIV', {
						props: {
						...
						children: [
							BX.create('LI', {
								children: [
									BX.create('DIV', {
										props: {
											className: 'bx-soa-pp-list-termin'
										},
										html: this.params.MESS_PRICE + ':'
									}),
									BX.create('DIV', {
										props: {
											className: 'bx-soa-pp-list-description'
										},
				...
				extPs = BX.create('DIV', {
					children: [subTitle, label, title, price]
				});
			}

			if (innerPs && extPs)
				delimiter = BX.create('HR', {
					props: {
						className: 'bxe-light'
					}
				});
		...
		selectPaySystem: function (event) {
			...
			var innerPsAction = BX.hasClass(target, 'bx-soa-pp-inner-ps') ? target : BX.findParent(target, {
					className: 'bx-soa-pp-inner-ps'
				}),
				actionSection = BX.hasClass(target, 'bx-soa-pp-company') ? target : BX.findParent(target, {
					className: 'bx-soa-pp-company'
				}),
		...
		editDeliveryItems: function (deliveryNode) {
			...
			var deliveryItemsContainer = BX.create('DIV', {
				...
				deliveryItemsContainerRow = BX.create('DIV', {
					props: {
						className: 'row'
					}
				}),
		...
		editDeliveryInfo: function (deliveryNode) {
			...
			var deliveryInfoContainer = BX.create('DIV', {
					props: {
						className: 'col-md-5 mb-lg-0 col-12 mb-3 order-md-2 order-1 bx-soa-pp-desc-container'
					}
				}),
			...
			for (i in delivery.EXTRA_SERVICES) {
				...
				if (currentService.editControl.indexOf('this.checked') == -1) {
					...
					serviceNode = BX.create('DIV', {
						props: {
							className: 'form-group bx-soa-pp-field'
						},
		...
		createDeliveryItem: function (item) {
			...
			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== 'undefined') {
				labelNodes.push(
					BX.create('DIV', {
						props: {
							className: 'bx-soa-pp-delivery-cost'
						},
				...
				labelNodes.push(
					BX.create('DIV', {
						...
						html: typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== 'undefined' ?
							deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED : deliveryCached.PRICE_FORMATED
					})
				);
			}
		...
		editFadeDeliveryContent: function (node) {
			...
			if (selectedDelivery && selectedDelivery.NAME) {
				...
				node.appendChild(
					BX.create('DIV', {
						...
						children: [
							BX.create('DIV', {
								props: {
									className: 'col-sm-9 bx-soa-pp-company-selected'
								},
								children: arNodes
							}),
							BX.create('DIV', {
								props: {
									className: 'col-sm bx-soa-pp-price'
								},
				...
				node.appendChild(BX.create('STRONG', {
		...
		editPickUpList: function (isNew) {
			...
			var pickUpList = BX.create('DIV', {
					props: {
						className: 'bx-soa-pickup-list main'
					}
				}),
			...
			if (!!isNew) {
				...
				container.appendChild(
					BX.create('DIV', {
						props: {
							className: 'bx-soa-pickup-subTitle'
						},
		...
		createPickUpItem: function (currentStore, options) {
			...
			storeNode = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {
						props: {
							className: 'bx-soa-pickup-l-item-adress'
						},
					...
					BX.create('DIV', {
						...
						children: [
							...
							BX.create('DIV', {
								props: {
									className: 'bx-soa-pickup-l-item-name'
								},
								text: currentStore.TITLE
							}),
							BX.create('DIV', {
								props: {
									className: 'bx-soa-pickup-l-item-desc'
								},
								html: html
							})
						]
		...
		editPickUpRecommendList: function (geoLocation) {
			...
			var recommendList = BX.create('DIV', {
					props: {
						className: 'bx-soa-pickup-list recommend'
					}
				}),
				buyerStoreInput = BX('BUYER_STORE'),
				selectedDelivery = this.getSelectedDelivery();
		...
		editActivePropsBlock: function (activeNodeMode) {
			var node = activeNodeMode ? this.propsBlockNode : this.propsHiddenBlockNode,
				propsContent, propsNode, selectedDelivery, showPropMap = false,
				i, validationErrors;
		...
		editPropsItems: function (propsNode) {
			...
			var propsItemsContainer = BX.create('DIV', {
					props: {
						className: 'col-sm-12 bx-soa-customer'
					}
				}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(),
				propsIterator;
		...
		getPropertyRowNode: function (property, propsItemsContainer, disabled) {
			...
			if (disabled) {
				...
				label = BX.create('LABEL', {
					attrs: {
						'for': 'soa-property-' + property.getId()
					},
					props: {
						className: 'bx-soa-custom-label'
					},
		...
		alterProperty: function (settings, propContainer) {
			var divs = BX.findChildren(propContainer, {
					tagName: 'DIV'
				}),
			...
			if (add.length) {
				...
				BX.bind(add, 'click', BX.delegate(function (e) {
					var target = e.target || e.srcElement,
						...
						divs = BX.findChildren(targetContainer, {
							tagName: 'DIV'
						});
		...
		alterDateProperty: function (settings, inputText) {
			var parentNode = BX.findParent(inputText, {
					tagName: 'DIV'
				}),
				appendNode;
			...
			appendNode = BX.create('DIV', {
				props: {
					className: 'input-group-append bx-no-alter-margin'
				},
				...
				if (this.activeSectionId == this.propsBlockNode.id) {
					...
					for (i = 0; i < tooltips.length; i++) {
						if (tooltips[i].getAttribute('data-state') == 'opened') {
							this.animateScrollTo(BX.findParent(tooltips[i], {
		...
		editPropsComment: function (propsNode) {
			...
			label = BX.create('LABEL', {
				...
				props: {
					className: 'bx-soa-customer-label'
				},
		...
		editTotalBlock: function () {
			...
			// Start coupon
			let coupon = BX.create('div', {
				attrs: {
					className: 'bx-soa-cart-total-line coupon',
				},
				text: ''
			});
			...
			coupon.appendChild(couponSubmit);
			this.totalInfoBlockNode.appendChild(coupon);
			...
			this.totalInfoBlockNode.appendChild(this.createTotalUnit("Количество товаров", total.BASKET_POSITIONS, {
				free: true
			}));

			// Start bonus

			let bonus = BX.create('div', {
				attrs: {
					className: 'bx-soa-cart-total-line bonus',
				}
			});

			let bonusLeft = BX.create('div', {
				attrs: {
					className: 'left',
				}
			});

			let bonusSvg = BX.create('span', {
				attrs: {
					className: 'use',
				},
				html: '<svg><use xlink:href="/local/templates/assets/img/sprite.svg#scores"></use></svg>'
			});

			let bonusTitle = BX.create('span', {
				attrs: {
					className: 'title',
				},
			...
			bonusLeft.appendChild(bonusTitle);
			...
			if (this.result.DELIVERY.length && deliveryValue != 'бесплатно') {
				// Стоимость доставки 
			...
			if (this.options.showPayedFromInnerBudget) {
				...
				this.totalInfoBlockNode.appendChild(this.createTotalUnit('Итого', total.ORDER_TOTAL_PRICE_FORMATED, {
			...
			this.totalInfoBlockNode.appendChild(BX.create('div', {
				...
				html: 'Нажимая на кнопку Оформить заказ, я соглашаюсь с <a>Политикой конфиденциальности</a> , даю согласие на обработку Персональных данных и принимаю <a>Условия обслуживания</a>'
			}));
		...
		totalBlockScrollCheck: function () {
			...
			if (scrollTop + 81 > ghostTop && !BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed')) {
			...
			} else if (scrollTop + 81 < ghostTop && BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed')) {

...
foreach ($arResult["SECTIONS"] as $section) {
	if ($section["UF_SHOW_MAIN_PAGE"]) {
		$sectionProps[] = array(
			"NAME" => $section["NAME"],
			"PICTURE" => $section["PICTURE"]['SRC'],
			"SECTION_PAGE_URL" => $section["SECTION_PAGE_URL"],
			"SORT" => $section["UF_SORT"],
			"UF_SHOW_MAIN_PAGE" => $section["UF_SHOW_MAIN_PAGE"]
		);
	}
}
$sortArr = array_column($sectionProps, "SORT");
array_multisort($sortArr, SORT_ASC, $sectionProps);

?>

<div class="categry-wrapper">

	<? foreach ($sectionProps as $key => $item) : ?>

		<?
		if (($key + 1) % 2 == 0) continue;
		?>

		<? if ($arResult['SECTIONS'][$key + 1]) : ?>
			<? $nextItem = $sectionProps[$key + 1] ?>

			<div class="category-items" <?= ($key >= 3) ? 'style="display:none;"' : '' ?>>
				...
				<div class="item">
					<div class="img">
						<a href="<?= $nextItem['SECTION_PAGE_URL'] ?>">
							<img src="<?= $nextItem['PICTURE'] ?>" alt="">
						</a>
					</div>
					<div class="name">
						<a href="<?= $nextItem['SECTION_PAGE_URL'] ?>">

...
<div class="bx-filter <?= $templateData["TEMPLATE_CLASS"] ?> <? if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") echo "bx-filter-horizontal" ?>">
	<div class="bx-filter-section container-fluid">
		...
		?>
				<? if (!$isActiveFilter) : ?> <div class="title">Сортировать по:</div> <? endif; ?>
				<form name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>" method="get" class="smartfilter <?= $isActiveFilter ? 'active' : '' ?>">
					<div class="<? if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") : ?>col-sm-6 col-md-4<? else : ?>col-12<? endif ?> mb-2 smart-filter-parameters-box bx-active" style="display: none;">
									...
									<div class="d-flex justify-content-between">
										<div class="form-group" style="width: calc(50% - 10px);">
											<div class="smart-filter-input-container">
												<input class="min-price form-control form-control-sm" type="number" name="<? echo $arItem["VALUES"]["MIN"]["CONTROL_NAME"] ?>" id="<? echo $arItem["VALUES"]["MIN"]["CONTROL_ID"] ?>" value="<? echo $arItem["VALUES"]["MIN"]["HTML_VALUE"] ?>" size="5" placeholder="<?= GetMessage("CT_BCSF_FILTER_FROM") ?>" onkeyup="smartFilter.keyup(this)" />
											</div>
										</div>

										<div class="form-group" style="width: calc(50% - 10px);">
											<div class="smart-filter-input-container">
												<input class="max-price form-control form-control-sm" type="number" name="<? echo $arItem["VALUES"]["MAX"]["CONTROL_NAME"] ?>" id="<? echo $arItem["VALUES"]["MAX"]["CONTROL_ID"] ?>" value="<? echo $arItem["VALUES"]["MAX"]["HTML_VALUE"] ?>" size="5" placeholder="<?= GetMessage("CT_BCSF_FILTER_TO") ?>" onkeyup="smartFilter.keyup(this)" />
											</div>
										</div>
									</div>
		...
		foreach ($arResult["ITEMS"] as $key => $arItem) {
			...
			if ($arItem["DISPLAY_TYPE"] != 'A' && $arItem["DISPLAY_TYPE"] != 'B') {
				...
				if (count($filters) == 1) {
					if ($title == 'Наличие') $title = $filters[0];
					else $title .= ' ' . $filters[0];
				}
				...
				<div>
					...
					<div class="bx-filter-parameters-box-title">
						<span class="bx-filter-parameters-box-hint hover"><span class="filter-title" onclick="smartFilter.hideFilterProps($(this).parent().parent())"><?= $title ?></span>
							...
							<? endif ?>
							<? if (empty($filters)) : ?>
								<i data-role="prop_angle" class="fa fa-angle-down" onclick="smartFilter.hideFilterProps($(this).parent().parent())"></i>
							<? else : ?>
								<svg class="filter-clear-icon" data-cat="<?= $key ?>">
	...
	<div class="hide-block" data-role="currentOption"></div>
							...
							?>
								...
								?>
									<div class="col-xs-12 values scrollbar" id="c<?= $key ?>">
										<div class="bx-filter-param-btn-inline">
											<? foreach ($arItem["VALUES"] as $val => $ar) : ?>
												...
												<label for="<?= $ar["CONTROL_ID"] ?>" data-role="label_<?= $ar["CONTROL_ID"] ?>" class="bx-filter-param-label <?= $class ?>" onclick="smartFilter.keyup(BX('<?= CUtil::JSEscape($ar["CONTROL_ID"]) ?>')); BX.toggleClass(this, 'bx-active');">
													<span class="bx-filter-param-btn bx-color-sl hover">
								...
								?>
									...
									<div class="col-xs-12 values scrollbar" id="c<?= $key ?>">
										<div class="radio">
											<label class="bx-filter-param-label <? if (!$isChecked) : ?>active<? endif; ?>" for="<? echo "all_" . $arCur["CONTROL_ID"] ?>">
										...
										<? foreach ($arItem["VALUES"] as $val => $ar) : ?>
											<div class="radio">
												<label data-role="label_<?= $ar["CONTROL_ID"] ?>" class="bx-filter-param-label <? if ($ar['CHECKED']) : ?>active<? endif; ?> hover" for="<? echo $ar["CONTROL_ID"] ?>">
			...
			<div class="clear_filter">
				<input type="submit" id="del_filter" name="del_filter" value="Очистить все фильтры" />
			</div>
			...
			if ($_GET['sort'] == 'catalog_PRICE_1') {
				...
				if ($_GET['method'] && $_GET['method'] != 'ASC') {
					$title = 'Цена по убыванию';
				}
			}

			if ($_GET['sort'] && $_GET['sort'] == 'DATE_CREATE') {
				$title = 'Новинки';
			}

			if (!$_GET['sort'] || $_GET['sort'] == 'SHOW_COUNTER') {
				$title = 'По популярности';
			}
			?>
			<div>
				...
				<div class="bx-filter-block" data-role="bx_filter_block">
					<div class="row bx-filter-parameters-box-container">
						...
						<div class="col-xs-12 values scrollbar">

							<div class="radio">
								<label for="sort1" class="bx-filter-param-label <? if ($_GET['sort'] == 'DATE_CREATE') echo "active" ?> hover">
									<span class="bx-filter-input-checkbox">
										<input type="radio" value="DATE_CREATE" name="sort" id="sort1" onclick="smartFilter.click(this)" />
										<span class="bx-filter-param-text">Новинки</span>
									</span>
								</label>
							</div>

							<div class="radio">
								<label for="sort2" class="bx-filter-param-label <? if ($_GET['sort'] == 'SHOW_COUNTER') echo "active" ?> hover">
									<span class="bx-filter-input-checkbox">
										<input type="radio" value="SHOW_COUNTER" name="sort" id="sort2" onclick="smartFilter.click(this)" />
										<span class="bx-filter-param-text">По популярности</span>
									</span>
								</label>
							</div>

							<div class="radio">
								<label class="bx-filter-param-label <? if ($_GET['sort'] == 'catalog_PRICE_1' && $_GET['method'] == 'desc') echo "active" ?> hover">
									<span class="bx-filter-input-checkbox">
										<input type="radio" value="catalog_PRICE_desc" name="sort" onclick="smartFilter.click(this)" />
										<span class="bx-filter-param-text">Цена по убыванию</span>
									</span>
								</label>
							</div>
							...
							<div class="radio">
								<label class="bx-filter-param-label <? if ($_GET['sort'] == 'catalog_PRICE_1' && $_GET['method'] != 'desc') echo "active" ?> hover">
									<span class="bx-filter-input-checkbox">
										<input type="radio" value="catalog_PRICE_asc" name="sort" onclick="smartFilter.click(this)" />
										<span class="bx-filter-param-text">Цена по возрастанию</span>
									</span>
								</label>
							</div>
			...
			<div>
				...
				<span class="bx-filter-container-modef"></span>
				<div class="bx-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)">
					<span class="bx-filter-parameters-box-hint"><?= $title ?> <i data-role="prop_angle" class="fa fa-angle-down"></i>
					</span>
				</div>
				...
				<div class="bx-filter-block" data-role="bx_filter_block">
					<div class="row bx-filter-parameters-box-container">
						<div class="col-xs-12 values">
							<div class="bx-filter-param-btn-inline">
								<input style="display: none" type="checkbox" name="arrFilter_21_1120914852" id="arrFilter_21_1120914852" value="Y">

								<label for="arrFilter_21_1120914852" data-role="label_arrFilter_21_1120914852" class="bx-filter-param-label <? if ($_GET['sort'] == 'DATE_CREATE') echo "active" ?>" onclick="smartFilter.keyup(BX('arrFilter_21_1120914852')); BX.toggleClass(this, 'bx-active');">
									<span class="bx-filter-param-btn bx-color-sl">
										<a href="?sort=DATE_CREATE" class="color-name">Новинки</a>
									</span>
								</label>

								<label for="arrFilter_21_1120914852" data-role="label_arrFilter_21_1120914852" class="bx-filter-param-label <? if (!$_GET['sort'] || $_GET['sort'] == 'SHOW_COUNTER') echo "active" ?>" onclick="smartFilter.keyup(BX('arrFilter_21_1120914852')); BX.toggleClass(this, 'bx-active');">
									<span class="bx-filter-param-btn bx-color-sl">
										<a href="?sort=SHOW_COUNTER" class="color-name">По популярности</a>
									</span>
								</label>

								<label for="arrFilter_21_1120914852" data-role="label_arrFilter_21_1120914852" class="bx-filter-param-label <? if ($_GET['sort'] == 'catalog_PRICE_1' && $_GET['method'] == 'desc') echo "active" ?>" onclick="smartFilter.keyup(BX('arrFilter_21_1120914852')); BX.toggleClass(this, 'bx-active');">
									<span class="bx-filter-param-btn bx-color-sl">
										<a href="?sort=catalog_PRICE_1&method=desc" class="color-name">Цена по убыванию</a>
									</span>
								</label>

								<label for="arrFilter_21_1120914852" data-role="label_arrFilter_21_1120914852" class="bx-filter-param-label <? if ($_GET['sort'] == 'catalog_PRICE_1' && $_GET['method'] != 'desc') echo "active" ?>" onclick="smartFilter.keyup(BX('arrFilter_21_1120914852')); BX.toggleClass(this, 'bx-active');">
									<span class="bx-filter-param-btn bx-color-sl">
										<a href="?sort=catalog_PRICE_1" class="color-name">Цена по возрастанию</a>
									</span>
								</label>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$('input').on('change', function() {
		if ($(this).attr('id')) {
			smartFilter.keyup(BX($(this).attr('id')));
		}
	});
</script>
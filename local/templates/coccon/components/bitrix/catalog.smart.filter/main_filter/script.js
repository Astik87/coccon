...
JCSmartFilter.prototype.click = function (checkbox) {
	...
	let ladel = $(`label[for="${$(checkbox).attr('id')}"]`);
	ladel.toggleClass('active');
...
JCSmartFilter.prototype.hideFilterProps = function (element) {
	...
	let isActive = $(element).parent().hasClass('bx-active');

	$('.bx-filter-parameters-box-title').find('i').removeClass('fa-angle-up');
	$('.bx-filter-parameters-box-title').find('i').addClass('fa-angle-down');

	$('.smartfilter .bx-active').find('.bx-filter-block').slideToggle();
	$('.smartfilter .bx-active').removeClass('bx-active');

	if (!isActive) {
		$(element).parent().addClass('bx-active');
...
function send() {
	$.ajax({
		url: '/catalog/?set_filter=y&amp;arrFilter_21_337235200=Y',
		method: 'get',
		success: (data) => {
			$('html').html(data);
		}

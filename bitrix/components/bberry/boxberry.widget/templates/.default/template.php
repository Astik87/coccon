...
$bxbOptions['address'] = Option::get('up.boxberrydelivery', 'BB_ADDRESS');
$bxbOptions['bb_custom_link'] = Option::get('up.boxberrydelivery', 'BB_CUSTOM_LINK');
$bxbOptions['bb_paid_person_ph'] = Option::get('up.boxberrydelivery', 'BB_PAID_PERSON_PH');
$bxbOptions['bb_paid_person_jur'] = Option::get('up.boxberrydelivery', 'BB_PAID_PERSON_JUR');
    ...
    function delivery(result){
        ...
        if (element != undefined){
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                data: {save_pvz_id:result.id, change_location:result.name},
                success:function(data){checkSelectPvz();}
            });
            let linkClass = '<?=Option::get('up.boxberrydelivery', 'BB_BUTTON')=='Y' ? '.bxbbutton' : '.bxblink';?>';
            $(linkClass).text('Boxberry: '+ result.address + " #" + result.id);
        }
        return false;
    }
        ...
        $.ajax({
            ...
            success: function(not_selected){
                ...
                if (not_selected==true){
                    ...
                    $('#soa-property-'+boxberry_delivery_profiles.module_addr_options[key].ID).attr('readonly', true);
                    bxb_errors[0]=('<?=Loc::getMessage("PVZ_REQUIRED");?>');
                }
	...
	function getLink()
	{
		$.ajax({
			...
			success: function(return_data){
				$('#'+bb_custom_link).html("");
				$('#'+bb_custom_link).append('<a href="#" onclick="boxberry.checkLocation(1);boxberry.open('+  makeWidgetString(return_data) +');return false;" ><?=Loc::getMessage("BB_CUSTOM_LINK");?></a>');
			}
		});
	}

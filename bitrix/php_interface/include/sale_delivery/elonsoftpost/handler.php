	...
	protected static function getPostProfiles()
	{
		return array(
			...
			"3" => array(
				"ID" => "3",
				"CODE" => "POST_WORLD",
				"Name" => Loc::getMessage('SALE_DLV_ELONSOFT_POST_PVZ_WORLD'),
				"ShortDescription" => Loc::getMessage('SALE_DLV_ELONSOFT_POST_PVZ_WORLD_SDESCR'),
				"Description" => Loc::getMessage('SALE_DLV_ELONSOFT_POST_PVZ_WORLD_DESCR'),
			),
			"4" => array(
				"ID" => "4",
				"CODE" => "COURIER_WORLD",
				"Name" => Loc::getMessage('SALE_DLV_ELONSOFT_POST_COURIER_WORLD'),
				"ShortDescription" => Loc::getMessage('SALE_DLV_ELONSOFT_POST_COURIER_WORLD_SDESCR'),
				"Description" => Loc::getMessage('SALE_DLV_ELONSOFT_POST_COURIER_WORLD_DESCR'),
			),
		);
	}
	...
	public function getProfilesDefaultParams()
	{
		...
		if(is_array($srvTypes))
		{
			...
			foreach($srvTypes as $profId => $params)
			{
				...
				elseif ($profId == 3)
				{
					$arFile = \CFile::MakeFileArray('/bitrix/php_interface/include/sale_delivery/elonsoftpost/world_delivery.jpg');
					$arFileds1 = \CFile::SaveFile($arFile, "sale/delivery/logotip");
				}
				elseif ($profId == 4)
				{
					$arFile = \CFile::MakeFileArray('/bitrix/php_interface/include/sale_delivery/elonsoftpost/world_courier.jpg');
					$arFileds1 = \CFile::SaveFile($arFile, "sale/delivery/logotip");
				}
				$result[] = array(
					"CODE" => $params['CODE'],
